...
#IncludeFigHere <- "/castles/nr/projects/2017/gkoutosg-variant-prediction/Laura/Magda/Imputed_20231126_1417/"
IncludeFigHere <- "/castles/nr/projects/2017/gkoutosg-variant-prediction/Laura/Magda/Oct_20231108_1935/"
#IncludeFigHere <- "/castles/nr/projects/2017/gkoutosg-variant-prediction/Laura/Magda/GeneticsX_20231130_1403/" #same results as Oct!!

#will do it with this and then final plots with above! (GeneticsX_20231130_1403)

#IncludeFigHere <- "/castles/nr/projects/2017/gkoutosg-variant-prediction/Laura/Magda/GeneticsX_20231130_1403/"
...
#load("stacks.RData") #uncomment this too
load("ModelingWorkflows.RData")
load("Modeling2data3mods.RData")

################### Store results in new place 

# Set the main directory where the files and directories will be created
mainDir <- "/rds/projects/g/gkoutosg-variant-prediction/Laura/Magda/"
# Set the name for the run or analysis
NameRun <- "Tiff_Github_NonInputedTop10_Magda"
# Create a subdirectory name based on the NameRun and current date/time
subDir <- paste0(sub('\\..*', '', NameRun), format(Sys.time(), '_%Y%m%d_%H%M'))
# Create the subdirectory within the main directory
dir.create(file.path(mainDir, subDir))
# Define the path where figures or plots will be saved
IncludeFinal <- file.path(mainDir, subDir)
...
for (i in unique(mae_stats$Model_Type)){
  ...
  theme_set(
    theme_bw() +
      theme(plot.title = element_text(hjust = -0.25, size = 16,
  ...
  PerfMAE[[i]] <- rest2 %>%
    ...
    labs(x = "loss of MAE", y = NULL)  + ggtitle(paste0("Algorithm: ", i)) +
    labs(title = Letters[count]) #+ theme_bw()
...
pdf(paste0(IncludeFinal, "/LB_PerfromanceMAEFinalFig.pdf"), 10, 7)
print(wrap_plots(PerfMAE))
dev.off()


library(patchwork)
pdf(paste0(IncludeFinal, "/LB_PerfromanceMAEFinalFig.pdf"), 13, 7)
...
#library(biomaRt)
#ensembl_snp1 = useMart(biomart="ENSEMBL_MART_ENSEMBL", dataset="hsapiens_snp", host="https://useast.ensembl.org") #useMart("ENSEMBL_MART_SNP", dataset="hsapiens_snp")
#ensembl_snp2 <- useMart("ENSEMBL_MART_ENSEMBL", dataset="hsapiens_gene_ensembl")
#
#listMarts(host="https://useast.ensembl.org")
#listMarts(host="http://asia.ensembl.org/index.html")
...
load("/rds/projects/g/gkoutosg-variant-prediction/Laura/Magda/Oct_20231108_1935/modified_table.RData")
...
for (j in unique(vi_rnks2b$Dataset)){
  ...
  pdf(paste0(IncludeFinal,"/LB_NewHeatmaps_",j,".pdf"), 15, 12)
  
  print(viz)
  
  dev.off()
  
  tiff(paste0(IncludeFinal,"/LB_NewHeatmaps_",j,".tif"), res = 600, 
       width =40, height = 33, units = "cm")
  
  print(viz)
  
  dev.off()
  
  #ggsave(paste0(IncludeFinal,"/LB_NewHeatmapSave_",j,".pdf"), plot = viz, device = "pdf") #deleted this!
...
SelectFeaturesHist <- SelectFeatures %>% 
  group_by(var) %>% 
  count() %>% 
  filter(!(var %in% c("ClinicalAge", "Clinical_EducationMeasurement","rs10224002_G_X1","rs10254101_T_X2","rs118186402_A_X2"))) %>% #"rs118186402_A_X2","rs17046359_G_X2","rs73353563_T_X2"))) %>%  #repeated
  #filter(!(var %in% c("ClinicalAge", "Clinical_EducationMeasurement","rs10224002_G_X1","rs10254101_T_X2", "rs118186402_A_X2","rs17046359_G_X2","rs73353563_T_X2"))) %>%  #repeated
  arrange(-n) 

fwrite(SelectFeaturesHist, file = paste0(IncludeFinal,"/LB_CountSelectedFeatures.csv"))

SelectFeatures <- SelectFeaturesHist$var[1:10] #%>% 
#SelectFeatures <- filter(SelectFeaturesHist, n>1)$var #var[1:15] #%>% 
...
WithCogAndSNPs4ModeAPOE <-read.csv("/rds/projects/g/gkoutosg-variant-prediction/Laura/Magda/ImputedDatae2e3.csv") 
#- Imputed run
#e2e3 out as no link to cognitive decline
#would need a third oe with genetic sex
print("Imputed")
...
ToMediate <- AllRec2J %>%
  dplyr::select(c(SelectFeatures), GeneralCog, Clinical_EducationMeasurement, ClinicalAge) #%>%
...
print(names(ToMediate))

for (i in setdiff(names(ToMediate), c("GCAS","CR (Education)",  "Age"))) { #"rs798507(GNA12)",
  ...
  ToMediate3 <- ToMediate %>% 
    ...
    dplyr::select(GCAS, 'CR (Education)', i, Age )
  
  #debugonce(psych::mediate)
  mod.glb[[i]] <-  psych::mediate(y = "GCAS", x = "CR (Education)", m= eval(i), z= "Age", data = ToMediate3, plot = TRUE,  n.iter = 10000)
...
save(Params, file = paste0(IncludeFinal, "/LB_Params.RData"))

save(mod.glb, file = paste0(IncludeFinal, "/LB_mod.glb.RData")) # Table with values better!
...
fwrite(s, file = paste0(IncludeFinal, "/LB_MediationDiagrams2Age.csv"))



print("Mediation Figures")

pdf(paste0(IncludeFinal, "/LB_MediateDiagramAll.pdf"), 10, 5)
...
pdf(paste0(IncludeFinal, "/LB_ContinousPLot.pdf"), 16,  9)
print(NetworksPlot)
dev.off()
...
pdf(paste0(IncludeFinal, "/LB_AllPlots.pdf"), 16,  15) #13
NetworksPlot/Genes
dev.off()
...
load("/castles/nr/projects/2017/gkoutosg-variant-prediction/Laura/Magda/SelectedParticipants3.RData") # not from imputed participants! Have to redo
#WithCog <- readRDS("/rds/projects/g/gkoutosg-variant-prediction/Laura/Magda/imputedWithCog.RDS")
...
pdf(paste0(IncludeFinal, "/LB_SFig1_MissingVal1.pdf"), 25, 30)
print(Miss1)
dev.off()
...
pdf(paste0(IncludeFinal, "/LB_SFig2_MissingVal.pdf"), 34, 23)
print(Miss1)
dev.off()


pdf(paste0(IncludeFinal, "/LB_Missing2.pdf"), 20, 10)
...
pdf(paste0(IncludeFinal, "/LB_PlotMissing.pdf"), 4,11)
